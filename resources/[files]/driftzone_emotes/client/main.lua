...
Citizen.CreateThread(function()
    ...
    favoriteAnimations = {}
    local kvp = GetResourceKvpString('driftzone_emotes_favorites_v2')
    ...
    quickAnimations = {}
    local kvp2 = GetResourceKvpString('driftzone_emotes_quicks_v2')
    if kvp2 then
...
RegisterNetEvent('driftzone_emotes:openMenu:client', openMenu)
...
RegisterNUICallback('callback', function(data)
    ...
    elseif data.action == "saveFavAnims" then
        SetResourceKvp("driftzone_emotes_favorites_v2", json.encode(data.favoriteAnimations))
    elseif data.action == "saveQuickAnims" then
        ...
        SetResourceKvp("driftzone_emotes_quicks_v2", json.encode(quickAnimationsTable))
        Citizen.Wait(250)
        setQuickKeys()
    elseif data.action == "keybindSaved" then
        ...
        SetResourceKvp("driftzone_emotes_quicks_v2", json.encode(keybindAnimations))
        Citizen.Wait(250)
        setQuickKeys()
    elseif data.action == "playAnim" then
        ...
        if data.category == "general" or data.category == "propemotes" or data.category == "animalemotes" or data.category == "extra" or data.category == "gang" then
            ...
            if animData.AnimationOptions and animData.AnimationOptions.Scenario or animData[1] == "Scenario" then
                ...
                if animData.AnimationOptions and animData.AnimationOptions.PtfxAsset then
                    ...
                    PtfxPrompt = true
                    RunAnimationThread()
                    TriggerServerEvent("driftzone_emotes:ptfxSync:server", PtfxAsset, PtfxName, vector3(Ptfx1, Ptfx2, Ptfx3), vector3(Ptfx4, Ptfx5, Ptfx6), PtfxBone, PtfxScale, PtfxColor)
                else
                    PtfxPrompt = false
                end
                ...
                if animData.AnimationOptions and animData.AnimationOptions.Prop then
                    ...
                    if animData.AnimationOptions.PtfxAsset and not PtfxNoProp then
                        TriggerServerEvent("driftzone_emotes:ptfxSyncProp:server", ObjToNet(prop))
                    end
                end
        ...
        elseif data.category == "placedemotes" then
            if Config.UseOldVersionPlacing then
                ...
                TriggerServerEvent('driftzone_emotes:setPedAlpha:server', GetPlayerServerId(PlayerId()), 200)
...
function cancelEmote(hu)
    if dzLockedEmote and not dzForceCancel then
        Notify('Acest emote este blocat. Poate fi oprit doar prin trigger-ul forceStop.', 5000, 'error')
        return false
    end
    dzLockedEmote = false
    dzLastLockedName = nil
    if isInAnimation then
        currentAnimData = {}
        isInAnimation = false
        ClearPedTasks(PlayerPedId())
        ...
        if lastPlayedAnimType == "synced" then
            ...
            if syncedTarget then
                ...
                TriggerServerEvent('driftzone_emotes:cancelEmote:server', syncedTarget)
            end
...
RegisterCommand('eclone', function(source, args, raw) EmoteCommandStart(source, args, raw, "clone") end, false) 
...
function EmoteCommandStart(source, args, raw, type)
    ...
    if #args > 0 then
        ...
        if dzLockedEmote and not dzForceCancel and type ~= 'dz_locked_force' then
            if requestedName == 'c' or requestedName == 'cancel' or requestedName == 'stop' then
                Notify('Acest emote este blocat. Poate fi oprit doar prin trigger-ul forceStop.', 5000, 'error')
            else
                Notify('Ai deja un emote blocat activ.', 5000, 'error')
            end
            return
        end
...
RegisterNetEvent('driftzone_emotes:EmoteCommandStart:client', function(data)
...
exports("EmoteCancel", function() return cancelEmote("driftzone") end)
                        ...
                        RegisterCommand('quickanim', function(source, args, raw)
                            if not keyLoop then
                                ...
                                Citizen.CreateThread(function()
                                    while keyLoop do
                                        Citizen.Wait(0)
                                        if IsControlPressed(0, Config.NumKeys[1].Key) or IsDisabledControlPressed(0, Config.NumKeys[1].Key) then 
                                        ...
                                        if IsControlPressed(0, Config.NumKeys[2].Key) or IsDisabledControlPressed(0, Config.NumKeys[2].Key) then 
                                        ...
                                        if IsControlPressed(0, Config.NumKeys[3].Key) or IsDisabledControlPressed(0, Config.NumKeys[3].Key) then 
                                        ...
                                        if IsControlPressed(0, Config.NumKeys[4].Key) or IsDisabledControlPressed(0, Config.NumKeys[4].Key) then 
                                        ...
                                        if IsControlPressed(0, Config.NumKeys[5].Key) or IsDisabledControlPressed(0, Config.NumKeys[5].Key) then 
                                        ...
                                        if IsControlPressed(0, Config.NumKeys[6].Key) or IsDisabledControlPressed(0, Config.NumKeys[6].Key) then 
                                        ...
                                        if IsControlPressed(0, Config.NumKeys[7].Key) or IsDisabledControlPressed(0, Config.NumKeys[7].Key) then 
...
RegisterNetEvent('driftzone_emotes:receiveAnimRequest:client', function(data)
    requestReceiveActive = true
    ShowTextUI(Lang:t("notifications.waiting_for_a_decision"), "ESC")
    Create3DTextUIOnPlayer("driftzone_emotes-request-player-" .. data.target, {
        id = data.target,
        displayDist = 5.0,
        interactDist = 1.3,
        enableKeyClick = true, 
        ...
        theme = "green", 
        triggerData = {
            triggerName = "driftzone_emotes:playAnimTogetherReceiver:client",
            args = {data = data}
        }
    })
    Citizen.CreateThread(function()
        while requestReceiveActive do
            ...
            if IsControlPressed(0, 322) then
                ...
                HideTextUI()
                Delete3DTextUIOnPlayer("driftzone_emotes-request-player-" .. data.target)
                TriggerServerEvent('driftzone_emotes:requstCanelledNotif:server', data.target)
                break
            end
        end
    end)
...
RegisterNetEvent('driftzone_emotes:animDictLoaded:client', function()
    syncedAnimDictLoaded = true
end)

RegisterNetEvent('driftzone_emotes:playAnimTogetherReceiver:client', function(data)
    requestReceiveActive = false
    requestActive = false
    ...
    HideTextUI()
    Delete3DTextUIOnPlayer("driftzone_emotes-request-player-" .. data.data.target)
    local inVehicle = IsPedInAnyVehicle(PlayerPedId(), true)
    if inVehicle then return end
    TriggerServerEvent('driftzone_emotes:playAnimTogetherSender:server', data)
    Citizen.Wait(300)
    ...
    TriggerServerEvent('driftzone_emotes:animDictLoaded:server', syncedTarget)
    local startTime = GetGameTimer()
...
RegisterNetEvent('driftzone_emotes:playAnimTogetherSender:client', function(data)
    requestReceiveActive = false
    requestActive = false
    lastPlayedAnimType = "synced"
        ...
        if animOptions.Attachto then
            ...
            if targetAnimData then
                if targetAnimData.Attachto then
                    TriggerServerEvent('driftzone_emotes:attachPeds:server', syncedTarget, GetPlayerServerId(PlayerId()), targetAnimData)
...
RegisterNetEvent('driftzone_emotes:attachPeds:client', function(targetId, targetAnimData)
...
RegisterNetEvent('driftzone_emotes:cancelEmote:client', function()
    cancelEmote("0res")
end)
...
RegisterNetEvent('driftzone_emotes:getAnimationList:client', function()
...
local function dzPlayByName(emoteName, locked)
    ...
    if locked then
        dzForceCancel = true
        cancelEmote('driftzone_force')
        dzForceCancel = false
        dzLockedEmote = false
        dzLastLockedName = nil
        EmoteCommandStart(0, { emoteName }, nil, 'dz_locked_force')
        if isInAnimation then
            dzLockedEmote = true
            dzLastLockedName = emoteName
        end
        return true
    end

    if dzLockedEmote then
        Notify('Ai deja un emote blocat activ.', 5000, 'error')
        return false
    end

    EmoteCommandStart(0, { emoteName }, nil, nil)
    return true
end

local function dzForceStop()
    dzForceCancel = true
    cancelEmote('driftzone_force')
    dzForceCancel = false
    dzLockedEmote = false
...
RegisterNetEvent('driftzone_emotes:client:open', function()
    openMenu()
end)

RegisterNetEvent('driftzone_emotes:client:play', function(emoteName)
    dzPlayByName(emoteName, false)
end)

RegisterNetEvent('driftzone_emotes:client:playLocked', function(emoteName)
    dzPlayByName(emoteName, true)
end)

RegisterNetEvent('driftzone_emotes:client:stop', function()
    cancelEmote('driftzone')
end)

RegisterNetEvent('driftzone_emotes:client:forceStop', function()
    dzForceStop()
end)

exports('OpenMenu', function() return openMenu() end)
exports('PlayEmote', function(emoteName) return dzPlayByName(emoteName, false) end)
exports('PlayLockedEmote', function(emoteName) return dzPlayByName(emoteName, true) end)
exports('CancelEmote', function(force) if force then return dzForceStop() end return cancelEmote('driftzone') end)
exports('ForceStopEmote', function() return dzForceStop() end)
exports('IsInEmote', function() return isInAnimation, { locked = dzLockedEmote, name = dzLastLockedName } end)
