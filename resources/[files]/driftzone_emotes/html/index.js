...
function createAnimationItem(item) {
    ...
    let imageFallbackStep = 0;
    img.onerror = () => {
        imageFallbackStep++;
        if (imageFallbackStep === 1) {
            img.src = `files/${imgName}.webp`;
            return;
        }
        if (imageFallbackStep === 2) {
            img.src = `files/${imgName}.png`;
            return;
        }
        if (imageFallbackStep === 3) {
            img.src = `assets/images/${imgName}.webp`;
            return;
        }
        if (imageFallbackStep === 4) {
...
typingSound.addEventListener('error', () => { if (!typingSound.dataset.fallback) { typingSound.dataset.fallback = '1'; typingSound.src = 'assets/sounds/writing.mp3'; } });
...
function handleDragDrop() {
    if (typeof $ === 'undefined' || !$.fn || !$.fn.draggable || !$.fn.droppable) { return; }
    // Normal
